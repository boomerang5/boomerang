...
import type { TranslationTokenData, TargetLanguageOption, LanguageOption } from './types'
...
export const DEFAULT_TRANSLATION_CONFIG = {
  sourceLang: 'es-ES',
  targetLang: 'en',
  voice: TARGET_LANGUAGE_OPTIONS[0].voices[0].value,
...
export class TranslationQueue {
  ...
  public add(text: string): void {
    ...
    console.log(`📝 [COLA] Agregado a cola: "${text}" (cola: ${this.queue.length})`)
  }

  public async process(voice: string, tokenData: TranslationTokenData): Promise<void> {
    ...
    console.log(`🎤 [COLA] Procesando: "${textToPlay}" (restantes: ${this.queue.length})`)

    try {
      await this.synthesizeAndPlay(textToPlay, voice, tokenData)
      console.log(`✅ [COLA] Completado: "${textToPlay}"`)
    } catch (error) {
  ...
  private async synthesizeAndPlay(text: string, voice: string, tokenData: TranslationTokenData): Promise<void> {
      ...
      synthesizer.speakTextAsync(
        ...
        async (result: SpeechSDK.SpeechSynthesisResult) => {
          try {
            ...
            console.log(`✅ [TTS] Completado en ${ttsLatency}ms`)

            synthesizer.close()

            if (result.reason !== SpeechSDK.ResultReason.SynthesizingAudioCompleted) {
              console.error('TTS failed:', (result as any).errorDetails)
              reject(new Error('Error en síntesis de voz: ' + ((result as any).errorDetails || 'Desconocido')))
              return
            }
            ...
            try {
              audioBuffer = await audioCtx.decodeAudioData(arrayBuf.slice(0) as ArrayBuffer)
            } catch {
              // Fallback a callback API
              audioBuffer = await new Promise<AudioBuffer>((res, rej) => {
                audioCtx.decodeAudioData(arrayBuf.slice(0) as ArrayBuffer, res, rej)
              })
            }
            ...
            reject(new Error('Error procesando audio TTS: ' + (err as Error).message))
          }
        },
        (error: string) => {
...
export function createTranslationRecognizer(
  ...
  const stConfig = SpeechSDK.SpeechTranslationConfig.fromAuthorizationToken(tokenData.token, tokenData.region)
...
export function createPeerAudioConfig(remoteVideoRef: React.RefObject<HTMLVideoElement | null>): {
  ...
  const remoteAudioContext = new (window.AudioContext || (window as any).webkitAudioContext)()

  // Crear un stream que contenga SOLO el audio del peer
  const peerOnlyStream = new MediaStream([remoteAudioTrack])
  const remoteAudioSource = remoteAudioContext.createMediaStreamSource(peerOnlyStream)
  const remoteAudioDestination = remoteAudioContext.createMediaStreamDestination()
