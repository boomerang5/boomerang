...
export default function CallNotificationsProvider({
  children,
  callRoute = '/protected/videollamada', // <-- cambia si tu ruta es otra
  ...
  // 2) Identidad (uuid + id numérico cacheado) - SOLO cuando la sesión esté lista
  useEffect(() => {
    if (!sb || !user) return
    
    log('🔍 Resolviendo identidad con sesión lista...')
    
    ;(async () => {
      let uuid: string | null = null
      
      // PRIORIDAD 1: UUID de sesión autenticada
      if (user?.id) {
        uuid = user.id
        log('✅ UUID desde useUser():', uuid)
      }
      
      // PRIORIDAD 2: RPC get_usuario_uuid (si no hay sesión)
      if (!uuid) {
        try {
          log('🔄 Intentando RPC get_usuario_uuid...')
          ...
          if (rpcUuid) {
            uuid = rpcUuid
            log('✅ UUID desde RPC get_usuario_uuid:', uuid)
          } else {
            log('⚠️ RPC get_usuario_uuid devolvió:', uuidData)
          }
        } catch (e) {
          log('❌ Error en RPC get_usuario_uuid:', e)
        }
      }
      
      // PRIORIDAD 3: Fallback por pestaña (solo si no hay UUID real)
      if (!uuid) {
        const ss = sessionStorage.getItem('vc_uuid')
        if (ss) {
          uuid = ss
          log('⚠️ Usando UUID de sessionStorage (fallback):', uuid)
        } else {
          const g = (typeof crypto !== 'undefined' && 'randomUUID' in crypto) ? crypto.randomUUID() : Math.random().toString(36).slice(2)
          sessionStorage.setItem('vc_uuid', g)
          uuid = g
          log('⚠️ Generando nuevo UUID por pestaña:', uuid)
        }
      }

      if (!uuid) return
      setMeUuid(uuid)
      log('🎯 meUuid final set to:', uuid)
      ...
      log('meNumericId set to:', idNum)
    })()
  }, [sb, user])

  // 3) Suscripción global a user:<uuid> y user:<id>
  useEffect(() => {
    if (!sb) return
    ...
    ;(async () => {
      ...
      const setup = async (key: string) => {
        ...
        ch.on('broadcast', { event: 'ring' }, ({ payload }) => {
          ...
          setIncoming({ callId: payload.callId, fromId, fromName })
          try { navigator.vibrate?.(200) } catch {}
        })
        ch.on('broadcast', { event: 'cancel' }, ({ payload }) => {
          log('← cancel on', `user:${key}`, payload)
        ...
        ch.on('broadcast', { event: 'reject' }, ({ payload }) => {
          log('← reject on', `user:${key}`, payload)
          if (payload.callId !== currentCallIdRef.current) return
          setIncoming(null); currentCallIdRef.current = null; peerIdRef.current = null
        })
      ...
      // Escuchar por UUID autenticado
      if (meUuid) await setup(meUuid)
      // Escuchar también por vc_uuid por si el caller envía al UUID de pestaña
      try {
  ...
  const onAccept = () => {
    ...
    if (!callId || !from) { log('! accept without callId/from', { callId, from }); return }
