...
export async function get_call_info(req: Request, res: Response) {
  ...
  if (Number.isNaN(idUsuario) || Number.isNaN(idLlamada)) {
    return res
      .status(400)
      .json({
        error: "Parámetros inválidos: idUsuario e idLlamada deben ser enteros.",
      });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error retrieving call info" });
  }
}
...
export async function get_user_call_history(req: Request, res: Response) {
  ...
  if (Number.isNaN(idUsuario)) {
    return res
      .status(400)
      .json({ error: "Parámetro inválido: id_usuario debe ser un número." });
  }
  ...
  try {
    ...
    if (!data || data.length === 0) {
      return res
        ...
        .json({
          success: true,
          data: [],
          message: "No se encontraron llamadas para este usuario.",
        });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error retrieving user call history" });
  }
}
