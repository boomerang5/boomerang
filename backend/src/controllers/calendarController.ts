    ...
    if (invitados && Array.isArray(invitados) && invitados.length > 0) {
      try {
        ...
        const organizadorNombre = creatorData
          ? `${creatorData.nombre || ""} ${creatorData.apellido || ""}`.trim() ||
            creatorData.apodo ||
            `Usuario ${id_creador}`
          : `Usuario ${id_creador}`;
        ...
        console.error(
          "⚠️ Error al crear notificaciones para nuevo evento (evento creado exitosamente):",
          notificationError
        );
      }
    }
...
export const delete_event = async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      const eventDetailsArray = await getEventDetailsService(
        Number(id_evento),
        Number(id_editor)
      );
      ...
      if (
        eventDetailsArray &&
        Array.isArray(eventDetailsArray) &&
        eventDetailsArray.length > 0
      ) {
        ...
        const idsInvitados = eventDetailsArray
          .filter((row: any) => {
            return (
              row.id_invitado !== null && row.id_invitado !== Number(id_editor)
            );
          })
      ...
      console.warn(
        "⚠️ No se pudieron obtener detalles del evento para notificaciones:",
        detailsError
      );
    }
    ...
    if (eventDetails && invitados.length > 0) {
      try {
        ...
        const organizadorNombre = organizadorData
          ? `${organizadorData.nombre || ""} ${organizadorData.apellido || ""}`.trim() ||
            organizadorData.apodo ||
            `Usuario ${id_editor}`
          : `Usuario ${id_editor}`;
        ...
        const resultadoNotificaciones =
          await NotificationService.createMultipleEventCancelledNotifications(
            invitados,
            tituloEvento,
            organizadorNombre,
            Number(id_evento),
        ...
        console.error(
          "⚠️ Error al enviar notificaciones de cancelación (evento eliminado exitosamente):",
          notificationError
        );
    ...
    return res
      .status(200)
      .json({ message: "Evento eliminado correctamente", result });
  } catch (err: any) {
    console.error("❌ Error inesperado:", err);
    return res
      .status(500)
...
export const get_event_details = async (req: Request, res: Response) => {
  ...
  if (!Number.isInteger(id_evento) || !Number.isInteger(id_usuario)) {
    return res
      ...
      .json({
        error:
          "Parámetros inválidos: id_evento e id_usuario deben ser enteros.",
      });
  }
...
export const get_user_events = async (req: Request, res: Response) => {
  ...
  if (!Number.isInteger(id_usuario)) {
    return res
      .status(400)
      .json({ error: "Parámetro inválido: id_usuario debe ser un entero." });
  }

  try {
    const events = await getUserEventsService(
      id_usuario,
      fecha_desde,
      fecha_hasta
    );
...
export const respond_event_invite = async (req: Request, res: Response) => {
  ...
  if (!id_evento || !id_usuario || typeof confirmado !== "boolean") {
    return res
      .status(400)
      .json({ error: "Faltan campos requeridos o parámetros inválidos." });
  }
  try {
    // Responder a la invitación del evento
    await respondEventInviteService(
      Number(id_evento),
      Number(id_usuario),
      confirmado
    );
    ...
    try {
      ...
      const eventNotification = userNotifications.find(
        (notification: any) =>
          (notification.tipo === "meeting_invite" ||
            notification.tipo === "reinvite") &&
      ...
      console.error(
        "⚠️ Error al actualizar notificación (respuesta del evento registrada exitosamente):",
        notificationError
      );
    }
    ...
    return res
      .status(200)
      .json({ message: "Respuesta registrada correctamente" });
  } catch (err: any) {
    console.error("❌ Error inesperado:", err);
    return res
      .status(500)
...
export const update_event_confirmation = async (
  req: Request,
  res: Response
) => {
  ...
  try {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: "Error al actualizar confirmación" });
    }
...
export const update_event = async (req: Request, res: Response) => {
  ...
  process.stdout.write(
    "🎯 [CONTROLLER] update_event INICIADO - ¡FUNCIÓN EJECUTADA!\n"
  );
  process.stdout.write(
    `🔄 update_event - Datos: ${JSON.stringify(req.body)}\n`
  );
...
export const update_event_participants = async (
  req: Request,
  res: Response
) => {
  ...
  if (!id_evento || !id_editor) {
    return res
      .status(400)
      .json({ error: "id_evento e id_editor son requeridos." });
  }
  ...
  try {
    ...
    try {
      const eventDetailsRaw = await getEventDetailsService(
        Number(id_evento),
        Number(id_editor)
      );
      ...
      if (Array.isArray(eventDetailsRaw)) {
        process.stdout.write(
          "📄 Los detalles del evento son un array, usando el primer elemento\n"
        );
      ...
      if (eventDetails && eventDetails.invitados) {
        participantesAnteriores = Array.isArray(eventDetails.invitados)
          ? eventDetails.invitados
              .map((inv: any) => inv.id)
              .filter((id: any) => id !== Number(id_editor))
          : [];
        ...
        const idsParticipantes = eventDetailsRaw
          .filter(
            (row: any) =>
              row.id_invitado && row.id_invitado !== Number(id_editor)
          )
          .map((row: any) => row.id_invitado);
      ...
      process.stdout.write(
        "⚠️ No se pudieron obtener participantes anteriores:\n"
      );
    ...
    process.stdout.write(
      "✅ update_event_participants - Participantes actualizados correctamente\n"
    );

    // Gestionar notificaciones de forma inteligente
    try {
      process.stdout.write(
        "🔍 Obteniendo detalles del evento para gestionar notificaciones...\n"
      );
      const eventDetailsRaw = await getEventDetailsService(
        Number(id_evento),
        Number(id_editor)
      ...
      if (Array.isArray(eventDetailsRaw) && eventDetailsRaw.length > 0) {
        process.stdout.write(
          "📄 Los detalles del evento son un array, usando el primer elemento para datos básicos\n"
        );
        eventDetails = eventDetailsRaw[0];
      }
      ...
      if (eventDetails) {
        ...
        const organizadorNombre =
          eventDetails.creador_nombre ||
          eventDetails.creator_name ||
          eventDetails.nombre_creador ||
          eventDetails.nombre ||
          `Usuario ${id_editor}`;
        const tituloEvento =
          eventDetails.titulo ||
          ...
          "Evento sin título";
        const fechaEvento =
          eventDetails.fecha || eventDetails.date || eventDetails.fechaHora;

        // Participantes nuevos (sin incluir al organizador)
        const participantesNuevos = (participantes || []).filter(
          (p: number) => p !== Number(id_editor)
        );
        process.stdout.write("👤 Participantes nuevos (sin organizador):\n");
        process.stdout.write(
          JSON.stringify(participantesNuevos, null, 2) + "\n"
        );

        // Determinar qué usuarios fueron removidos y cuáles agregados
        const participantesRemovidosIds = participantesAnteriores.filter(
          (id: number) => !participantesNuevos.includes(id)
        );
        const participantesAgregadosIds = participantesNuevos.filter(
          (id: number) => !participantesAnteriores.includes(id)
        );

        process.stdout.write("🗑️ Participantes removidos:\n");
        process.stdout.write(
          JSON.stringify(participantesRemovidosIds, null, 2) + "\n"
        );

        // 1. Eliminar notificaciones de invitación para usuarios removidos
        if (participantesRemovidosIds.length > 0) {
          const deleteResult =
            await NotificationService.deleteEventInviteNotificationsForUsers(
              Number(id_evento),
              participantesRemovidosIds
            );
          process.stdout.write(
            `✅ Eliminadas ${deleteResult.count} notificaciones de invitación\n`
          );
        }
        ...
        if (participantesAgregadosIds.length > 0) {
          process.stdout.write(
            "📧 Creando notificaciones para participantes agregados...\n"
          );
          const createResult =
            await NotificationService.createMultipleEventInviteNotifications(
          ...
          process.stdout.write(
            `✅ Creadas ${participantesAgregadosIds.length} notificaciones de invitación\n`
          );
        }

        if (
          participantesRemovidosIds.length === 0 &&
          participantesAgregadosIds.length === 0
        ) {
          process.stdout.write(
            "ℹ️ No hay cambios en participantes, no se modifican notificaciones\n"
          );
        }
      } else {
        process.stdout.write(
          "⚠️ No se obtuvieron detalles del evento para gestionar notificaciones\n"
        );
      }
    } catch (notificationError) {
      // Si falla la creación de notificaciones, no queremos que falle toda la operación
      console.error(
        "⚠️ Error al crear notificaciones (operación principal exitosa):",
        notificationError
      );
    }
    ...
    return res
      .status(200)
      .json({ message: "Participantes actualizados correctamente" });
  } catch (err: any) {
